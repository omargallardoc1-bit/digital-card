(()=>{
  ...
  async function list(cardId){
    ...
    let query=client.from('nfc_tags').select('id,card_id,label,token,placement,chip_type,status,notes,tap_count,last_tapped_at,created_at').order('created_at',{ascending:false});
  ...
  async function loadCards(){
    const client=db();
    if(!client)throw new Error('No se pudo conectar con la sesión del panel.');
    const {data,error}=await client.from('digital_cards').select('id,name,slug,status').eq('status','published').order('created_at',{ascending:false});
    if(error)throw error;
    return data||[];
  }
  ...
  async function create({cardId,label,placement='paper_card',notes=''}){
    ...
    if(userError||!user)throw new Error('Inicia sesión para crear un NFC.');
