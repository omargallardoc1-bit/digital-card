(()=>{
  ...
  async function load(id){
    if(!ensure()||!id||!state.organization?.id)return;
    ...
    if(error){state.crmActivities[id]=[];return;}
    state.crmActivities[id]=Array.isArray(data)?data:[];
    inject();
  }
  ...
  function inject(){
    if(!ensure())return;
    const editor=document.querySelector('.crm-editor');
    const id=state.crmSelectedProspectId;
    if(!editor||!id)return;
    let section=editor.querySelector('.crm-history-section');
    if(!section){
      section=document.createElement('section');section.className='crm-history-section';
      const form=editor.querySelector('form');
      if(form)form.insertAdjacentElement('afterend',section);else editor.appendChild(section);
    }
  ...
  window.refreshCrmHistory=()=>{const id=state?.crmSelectedProspectId;if(id)load(id)};
