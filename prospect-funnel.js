(()=>{
  ...
  function selectedCard(){const candidate=state?.metricsCard;return candidate&&candidate!=='all'?candidate:null}
  ...
  async function loadFunnel(force=false){
    ...
    funnelData=Array.isArray(data)?data[0]:data;funnelOrg=org;funnelCard=card;
  ...
  function funnelHtml(){
    ...
    const d=funnelData,steps=[[t('Prospectos','Prospects'),d.prospects],[t('Contactados','Contacted'),d.contacted],[t('Seguimiento','Follow-up'),d.follow_up],[t('Clientes','Customers'),d.customers]];
    return `<section class="dashboard-section mx-funnel-section"><div class="dashboard-section-header"><div><h2>${esc(t('Embudo comercial','Sales funnel'))}</h2><p>${esc(t('Del contacto captado al cliente.','From captured contact to customer.'))}</p></div><button class="ghost" type="button" onclick="window.refreshProspectFunnel()">${uiIcon('refresh')}<span>${esc(t('Actualizar','Refresh'))}</span></button></div><div class="dashboard-section-body"><div class="mx-funnel-grid">${steps.map(([label,value],i)=>`<div class="mx-funnel-step"><span>${esc(label)}</span><strong>${esc(num(value))}</strong>${i<steps.length-1?'<b aria-hidden="true">›</b>':''}</div>`).join('')}</div><div class="mx-funnel-summary"><div><span>${esc(t('Nuevos','New'))}</span><strong>${esc(num(d.new))}</strong></div><div><span>${esc(t('Trabajados','Worked'))}</span><strong>${esc(num(d.worked))}</strong></div><div><span>${esc(t('Descartados','Discarded'))}</span><strong>${esc(num(d.discarded))}</strong></div><div><span>${esc(t('Prospecto → cliente','Prospect → customer'))}</span><strong>${esc(String(d.prospect_to_customer_rate||0))}%</strong></div></div></div></section>`;
  }

  function styles(){
    if(document.getElementById('mx-funnel-style'))return;
    const s=document.createElement('style');s.id='mx-funnel-style';s.textContent=`.mx-funnel-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.mx-funnel-step{position:relative;min-height:104px;padding:16px;border:1px solid var(--border-default);border-radius:var(--radius-card);background:var(--surface-card);display:grid;align-content:center}.mx-funnel-step span{font-size:12px;font-weight:700;color:var(--text-muted)}.mx-funnel-step strong{font-size:26px;line-height:34px}.mx-funnel-step>b{position:absolute;right:-11px;top:35px;z-index:2;width:20px;height:32px;display:grid;place-items:center;color:var(--app-primary);font-size:28px;background:var(--surface-canvas)}.mx-funnel-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:12px}.mx-funnel-summary>div{padding:12px;border-radius:var(--radius-control);background:var(--surface-muted)}.mx-funnel-summary span,.mx-funnel-summary strong{display:block}.mx-funnel-summary span{font-size:11px;color:var(--text-muted);font-weight:700}.mx-funnel-summary strong{font-size:18px;margin-top:3px}@media(max-width:767px){.mx-funnel-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.mx-funnel-step>b{display:none}.mx-funnel-summary{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:420px){.mx-funnel-grid,.mx-funnel-summary{grid-template-columns:1fr}}`;document.head.appendChild(s)
  }
  ...
  if(typeof originalDashboard==='function'){
    window.dashboard=function(){
      styles();
      const html=originalDashboard.apply(this,arguments),funnel=funnelHtml(),pos=html.lastIndexOf('</div>');
      queueMicrotask(()=>loadFunnel());
      return pos>=0?html.slice(0,pos)+funnel+html.slice(pos):html+funnel;
    };
  } else {
    console.warn('MX prospect funnel: dashboard hook unavailable');
  }
})();
