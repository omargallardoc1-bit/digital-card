(()=>{
  'use strict';
  if(typeof state==='undefined'||typeof db==='undefined'||window.__crmHistorySafeInstalled)return;
  ...
  function historyHtml(id){
    ...
    if(!items)return `<section class="crm-safe-history"><div class="crm-safe-history-head"><h3>${esc(t('Historial comercial','Commercial history'))}</h3></div><div class="crm-safe-history-empty">${esc(t('Abre este prospecto para cargar su historial.','Open this prospect to load its history.'))}</div></section>`;
