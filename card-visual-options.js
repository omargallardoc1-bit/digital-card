...
(() => {
  ...
  const themeFor=card=>{const t=card?.theme||{},p=PRESETS[t.color_preset]||PRESETS.classic_mx;const btn=validHex(t.button_color)?t.button_color:p.btn;const bg=validHex(t.background_color)?t.background_color:p.bg;return{preset:t.color_preset||'classic_mx',bg,btn,text:validHex(t.button_text_color)?t.button_text_color:autoText(btn),bgText:autoText(bg)}};
  ...
  const css=document.createElement('style');css.textContent=`
    ...
    .card-cover .hero-logo{position:absolute!important;right:20px!important;left:auto!important;top:20px!important;width:auto!important;height:auto!important;max-width:min(28%,180px)!important;max-height:min(24%,96px)!important;object-fit:contain!important;object-position:center!important;display:block!important;box-sizing:border-box!important}
    @media(max-width:520px){.card-cover .hero-logo{right:14px!important;top:14px!important;max-width:30%!important;max-height:22%!important}}
    ...
    .screen .action,.public-card .action,.screen .card-main-action,.public-card .card-main-action,.screen .card-secondary-action,.public-card .card-secondary-action{background:var(--mx-button,#16A34A)!important;border-color:var(--mx-button,#16A34A)!important;color:var(--mx-button-text,#FFFFFF)!important}
    .screen .service-public-item,.public-card .service-public-item{background:var(--mx-card-bg,#FFFFFF)!important;color:var(--mx-card-text,#111827)!important}
    .screen .service-public-item strong,.public-card .service-public-item strong{color:var(--mx-card-text,#111827)!important}
    .screen .service-public-item p,.public-card .service-public-item p{color:var(--mx-card-text,#111827)!important;opacity:.78}
  `;document.head.appendChild(css);

  function applyVisualMarkup(markup,card){const h=normalizeHeight(card?.banner_height),t=themeFor(card);let html=String(markup).replace('class="hero card-cover" style="',`class="hero card-cover" data-banner-height="${h}" style="min-height:${h}px;height:${h}px;`);html=html.replace('class="screen"',`class="screen" style="--mx-card-bg:${t.bg};--mx-card-text:${t.bgText};--mx-button:${t.btn};--mx-button-text:${t.text};background:${t.bg}"`);html=html.replace('class="card-profile"',`class="card-profile" style="--mx-card-bg:${t.bg};--mx-card-text:${t.bgText};--mx-button:${t.btn};--mx-button-text:${t.text};background:${t.bg}"`);return html}
  ...
  function colorMarkup(){const t=themeFor(state?.card);const choices=Object.entries(PRESETS).map(([id,p])=>`<button type="button" class="palette-choice" data-preset="${id}" aria-pressed="${t.preset===id}"><span class="palette-swatches"><i style="background:${p.bg}"></i><i style="background:${p.btn}"></i></span><span>${p.name}</span></button>`).join('');return `<div class="color-theme-control" id="color-theme-control"><label>Colores de la tarjeta</label><div class="palette-grid">${choices}<button type="button" class="palette-choice" data-preset="custom" aria-pressed="${t.preset==='custom'}"><span class="palette-swatches"><i style="background:${t.bg}"></i><i style="background:${t.btn}"></i></span><span>Personalizado</span></button></div><div class="custom-colors ${t.preset==='custom'?'active':''}" id="custom-colors"><label>Fondo<input id="mx-bg-color" type="color" value="${t.bg}"></label><label>Botones<input id="mx-btn-color" type="color" value="${t.btn}"></label></div><small>Todos los botones usan el color elegido; los iconos y el texto se ajustan automáticamente para mantener contraste.</small></div>`}

  function applyThemeToRenderedCard(card){if(!card)return;const t=themeFor(card);document.querySelectorAll('.screen,.public-card .card-profile').forEach(el=>{el.style.setProperty('--mx-card-bg',t.bg);el.style.setProperty('--mx-card-text',t.bgText);el.style.setProperty('--mx-button',t.btn);el.style.setProperty('--mx-button-text',t.text);el.style.background=t.bg});document.querySelectorAll('.screen .action,.public-card .action,.screen .card-main-action,.public-card .card-main-action,.screen .card-secondary-action,.public-card .card-secondary-action').forEach(el=>{el.style.setProperty('background',t.btn,'important');el.style.setProperty('border-color',t.btn,'important');el.style.setProperty('color',t.text,'important')});document.querySelectorAll('.screen .service-public-item,.public-card .service-public-item').forEach(el=>{el.style.setProperty('background',t.bg,'important');el.style.setProperty('color',t.bgText,'important')});const row=document.querySelector('.public-language-row');if(row)row.style.background=t.bg}
  ...
  function installControls(){if(typeof state==='undefined'||state.page!=='editor')return;const identity=document.querySelector('[data-editor-block="identity"] .editor-block-content');if(!identity)return;if(!document.getElementById('banner-height-control')){identity.insertAdjacentHTML('beforeend',heightMarkup());document.getElementById('card-banner-height')?.addEventListener('change',async e=>{const h=normalizeHeight(e.target.value);state.card.banner_height=h;refreshPreview?.();if(state.card.id){const {error}=await db.rpc('set_card_banner_height',{target_card_id:state.card.id,requested_height:h});if(error)toast?.('No se pudo guardar la altura: '+error.message);else toast?.('Altura del banner guardada.')}})}
    ...
    document.querySelectorAll('.palette-choice').forEach(b=>b.addEventListener('click',()=>{const id=b.dataset.preset;document.querySelectorAll('.palette-choice').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));if(id==='custom'){const t=themeFor(state.card);setLocalTheme('custom',t.bg,t.btn);document.getElementById('custom-colors')?.classList.add('active');return}document.getElementById('custom-colors')?.classList.remove('active');const p=PRESETS[id];setLocalTheme(id,p.bg,p.btn);persistTheme(id,p.bg,p.btn)}));
  ...
  function applyPublicTheme(){if(typeof isPublicRoute!=='function'||!isPublicRoute())return;document.body.classList.add('mx-public-card');applyThemeToRenderedCard(state?.publicCard||state?.card)}
  ...
  let queued=false;const enhance=()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;installControls();installLanguage();applyPublicTheme();if(typeof state!=='undefined'&&state.page==='editor')applyThemeToRenderedCard(state.card)})};new MutationObserver(enhance).observe(document.documentElement,{childList:true,subtree:true});document.addEventListener('DOMContentLoaded',enhance);enhance();
})();
