(()=>{
  ...
  const reviewState={publicRows:[],adminRows:[]};
  ...
  async function loadPublicReviews(card){
    if(!card?.id)return [];
    const {data,error}=await db.from('card_reviews').select('id,card_id,service_id,reviewer_name,rating,comment,status,created_at,owner_reply,card_services(title)').eq('card_id',card.id).eq('status','approved').order('created_at',{ascending:false}).limit(30);
